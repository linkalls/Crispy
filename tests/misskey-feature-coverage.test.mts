...
import { buildMisskeyMultipartEntries, getMockMisskeyResponse } from '../src/utils/misskeyMock.ts';
import { incrementNoteRenoteLocally, toggleNoteReactionLocally } from '../src/utils/formatting.ts';
import { buildMisskeyExplorerPresetsFromDefinitionText, extractMisskeyEndpoints } from '../src/utils/misskeyExplorerGenerator.ts';
...
test('misskey explorer presets cover main feature groups', () => {
  ...
  const titles = new Set(MISSKEY_EXPLORER_PRESETS.map((preset) => `${preset.category}:${preset.title}`));
  assert.equal(titles.size, MISSKEY_EXPLORER_PRESETS.length);
  assert.ok(MISSKEY_EXPLORER_PRESETS.length > 400);
  assert.ok(MISSKEY_EXPLORER_PRESETS.some((preset) => preset.endpoint === '/api/notes/create'));
  assert.ok(MISSKEY_EXPLORER_PRESETS.some((preset) => preset.endpoint === '/api/channels/timeline'));
  assert.ok(MISSKEY_EXPLORER_PRESETS.some((preset) => preset.endpoint === '/api/federation/instances'));
});
...
test('generator extracts misskey endpoint definitions', () => {
  ...
  assert.deepEqual(extractMisskeyEndpoints(sample), ['channels/timeline', 'federation/instances', 'notes/timeline']);
...
test('normalizeMisskeyExplorerEndpoint adds /api prefix', () => {
  assert.equal(normalizeMisskeyExplorerEndpoint('notes/timeline'), '/api/notes/timeline');
  assert.equal(normalizeMisskeyExplorerEndpoint('/api/notes/timeline'), '/api/notes/timeline');
  assert.equal(normalizeMisskeyExplorerEndpoint('  /notes/search  '), '/api/notes/search');
});
...
test('parseMisskeyExplorerPayload accepts objects and rejects arrays', () => {
  ...
  assert.deepEqual(parseMisskeyExplorerPayload('{"limit":20}'), { ok: true, value: { limit: 20 } });
...
test('mock misskey responses cover common screens', () => {
  const timeline = getMockMisskeyResponse('notes/timeline', { host: 'misskey.io', payload: {} });
  assert.ok(Array.isArray(timeline));
  assert.ok((timeline as any[])[0].id);

  const notifications = getMockMisskeyResponse('i/notifications', { host: 'misskey.io', payload: {} });
  assert.ok(Array.isArray(notifications));
  assert.equal((notifications as any[])[0].type, 'reaction');

  const user = getMockMisskeyResponse('users/show', { host: 'misskey.io', payload: { userId: 'x' } });
  assert.equal((user as any).username, 'crispy');

  const uploaded = getMockMisskeyResponse('drive/files/create', { host: 'misskey.io', payload: {} });
...
test('multipart entries keep files and stringify scalars', () => {
  const entries = buildMisskeyMultipartEntries(
    {
      file: { uri: 'file:///tmp/image.png', name: 'image.png', type: 'image/png' },
      force: true,
      limit: 10,
      name: 'hello',
      ignored: null,
  ...
  assert.deepEqual(entries[1], ['file', { uri: 'file:///tmp/image.png', name: 'image.png', type: 'image/png' }]);
  assert.deepEqual(entries[2], ['force', 'true']);
  assert.deepEqual(entries[3], ['limit', '10']);
  assert.deepEqual(entries[4], ['name', 'hello']);
  assert.equal(entries.some(([key]) => key === 'ignored'), false);
});
...
test('note helpers keep local state consistent', () => {
  const note = {
    ...
    content: 'hello',
    createdAtLabel: '1m',
    user: { id: 'u1', name: 'User', username: 'user', host: 'misskey.io', avatar: '' },
    renoteUser: null,
