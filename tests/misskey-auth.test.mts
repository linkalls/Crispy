...
import { checkMiAuthSession, createMisskeyApiClient } from '../src/utils/misskeyAuth.ts';
...
test('checkMiAuthSession hits miauth session check', async () => {
  ...
  const result = await checkMiAuthSession('misskey.example', 'session-1', client as any);
  assert.equal(capturedUrl, 'https://misskey.example/api/miauth/session-1/check');
