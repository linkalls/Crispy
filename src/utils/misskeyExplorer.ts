...
export function groupMisskeyExplorerPresets(presets: MisskeyExplorerPreset[]) {
  return presets.reduce<Record<string, MisskeyExplorerPreset[]>>((groups, preset) => {
...
export function normalizeMisskeyExplorerEndpoint(input: string): string {
  ...
  return trimmed.startsWith('/api/') ? trimmed : `/api/${trimmed.replace(/^\/+/, '')}`;
}

export function parseMisskeyExplorerPayload(input: string): { ok: true; value: Record<string, unknown> } | { ok: false; error: string } {
  const trimmed = input.trim();
  ...
  try {
    ...
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
      return { ok: false, error: 'payload は object 形式の JSON である必要があります。' };
    }
