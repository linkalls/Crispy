...
function inferPayload(endpoint: string): Record<string, unknown> {
  if (endpoint.startsWith("notes/")) {
    ...
    if (endpoint.endsWith("/reactions/create"))
      return { noteId: "", reaction: "👍" };
    if (endpoint.endsWith("/reactions/delete")) return { noteId: "" };
    if (
      endpoint.endsWith("/timeline") ||
      endpoint.endsWith("/local-timeline") ||
      endpoint.endsWith("/global-timeline")
    )
      return { limit: 20 };
  }
  ...
  if (endpoint.startsWith("users/")) {
    ...
    if (endpoint.endsWith("/following") || endpoint.endsWith("/followers"))
  ...
  if (endpoint.startsWith("channels/")) {
    ...
    if (
      endpoint.endsWith("/show") ||
      endpoint.endsWith("/favorite") ||
      endpoint.endsWith("/unfavorite")
    )
      return { channelId: "" };
    if (endpoint.endsWith("/search")) return { query: "", limit: 20 };
  ...
  if (endpoint.startsWith("pages/")) {
    if (
      endpoint.endsWith("/show") ||
      endpoint.endsWith("/delete") ||
      endpoint.endsWith("/like") ||
      endpoint.endsWith("/unlike")
    )
      return { pageId: "" };
  ...
  if (endpoint.startsWith("flash/")) {
    ...
    if (
      endpoint.endsWith("/featured") ||
      endpoint.endsWith("/published") ||
      endpoint.endsWith("/my") ||
      endpoint.endsWith("/my-likes")
    )
      return { limit: 20 };
  }
...
export function extractMisskeyEndpoints(definitionText: string): string[] {
  ...
  return Array.from(
    new Set(Array.from(matches, (match) => match[1]).filter(Boolean)),
  ).sort();
}

export function buildMisskeyExplorerPresetsFromEndpoints(
  endpoints: string[],
): MisskeyExplorerPreset[] {
...
export function buildMisskeyExplorerPresetsFromDefinitionText(
  definitionText: string,
): MisskeyExplorerPreset[] {
  return buildMisskeyExplorerPresetsFromEndpoints(
    extractMisskeyEndpoints(definitionText),
  );
}
